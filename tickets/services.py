...
class MaximoEmailService:
    ...
    @classmethod
    def enviar_ticket_maximo(cls, ticket: Ticket, usuario: Cliente, arquivo_upload=None):
        """
        ...
        if arquivo_upload:
            try:
                ...
                content_type = getattr(arquivo_upload, 'content_type', 'application/octet-stream')
        ...
        try:
            ...
            logger.error(f"Erro crítico ao enviar e-mail para Maximo (Ticket {ticket.id}): {e}")
            # Opcional: Levantar exceção se quiser que a View trate o erro visualmente
            # raise e 

    @staticmethod
    def enviar_notificacao_chat(ticket: Ticket, interacao: TicketInteracao, autor: Cliente):
        """
        Envia notificação de interação (Chat).
        """
        email_suporte = getattr(settings, 'SUPPORT_EMAIL_ADDRESS', 'suportebr@itconsol.com')
        remetente = settings.DEFAULT_FROM_EMAIL
        ...
        if destinatarios:
            msg = EmailMessage(subject=assunto, body=corpo, from_email=remetente, to=destinatarios)
...
class NotificationService:
    ...
    def notificar_nova_interacao(cls, ticket: Ticket, interacao: TicketInteracao):
        ...
        email_suporte_geral = getattr(settings, 'SUPPORT_EMAIL_ADDRESS', 'suportebr@itconsol.com')
        
        # Texto de pré-visualização para a notificação do sininho
        preview_msg = f"{autor.first_name or autor.username}: {interacao.mensagem[:60]}..."
        ...
        if autor.is_support_team:
            ...
            assunto_email = f"[Portal Suporte] Nova resposta no Ticket #{ticket.maximo_id}"
            ...
            destinatarios_email = [email_suporte_geral] # E-mail vai para a caixa partilhada
